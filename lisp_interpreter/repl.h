...
Object *
builtin_sub_neg(Expression *call_exp,
                Object **env); // if 1 argument, return negation, else, subtract values in sequence
...
Object *builtin_eq(Expression *call_exp,
                   Object **env); // (eq a b) => compares if atoms are equivalent or if memory
                                  // addresses are the same
Object *builtin_cdr(Expression *call_exp,
                    Object **env); // (cdr list) => returns left side of a cons cell
Object *builtin_car(Expression *call_exp,
                    Object **env); // (car list) => returns remaining pair of list
Object *builtin_cons(Expression *call_exp,
                     Object **env); // (cons a b) => constructs cons cell from two elements
